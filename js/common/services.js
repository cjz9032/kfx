...
.service('IO', function($q, $http) {
		...
		this.SIDsToUri = function(sids) {
			...
			if (sids) {
				$http({
						method: 'GET',
						url: '/Handler/Filer.ashx?act=DownloadImage',
						params: {
							wxpid: window.wxpid,
							serverId: sids
						}
	...
	.service('WX', ['BASEURL_CONFIG',
		'$q',
		function($q,BASEURL_CONFIG) {
			var $this = this;
			this.share = function(shareData) {
				...
				if (shareData.imgUrl.indexOf('http') === -1) {
					shareData.imgUrl =  BASEURL_CONFIG + shareData.imgUrl;
...
//oauth
.service('oauth', function($q, principal, $http) {
		var $this = this;
		...
		this.logoutAll = function() {
			$http({
					method: 'GET',
					url: '/handler/User.ashx?act=Logout'
				})
					...
					return principal.identity()
						.then(function(_identity) {
							...
							if (isAuthenticated) {
								...
								console.log('未登录');
								var str = "AuthUser.aspx?wxpid=" + window.wxpid + "&fromurl=" + url;
								location.href = str;
							}
	...
	.filter("timeTip", function() {
		...
		var filterfun = function(time, isEnd) {
			var str = '';


			var r = time - nowTime;
			var r2 = Math.abs(r);
			var unit = Math.floor(r2 / SS.M) ? Math.floor(r2 / SS.M) + '个月' :
				Math.floor(r2 / SS.W) ? Math.floor(r2 / SS.W) + '个星期' :
				Math.floor(r2 / SS.D) ? Math.floor(r2 / SS.D) + '天' :
				Math.floor(r2 / SS.H) ? Math.floor(r2 / SS.H) + '小时' :
				Math.floor(r2 / SS.m) ? Math.floor(r2 / SS.m) + '分钟' :
				Math.floor(r2 / SS.s) ? Math.floor(r2 / SS.s) + '秒' : '1秒';
			if (isEnd) {
				if (r > 0) {
					str = '还有' + unit + '结束';
				} else {
	...
	.filter("getKeyName", function() {
		var filterfun = function(obj) {
			window.clearAllEmptyKeys(obj);
			var keyName = '';
			var arr = _.keys(obj).sort();
			_.each(arr, function(item, i) {
				if (!obj[item]) {
					return false;
				}
				if ((i + 1) === arr.length) {
					keyName += item + '=' + obj[item];
				} else {
					keyName += item + '=' + obj[item] + '&';
				}
...
window.clearAllEmptyKeys = function(obj) {
	for (var key in obj) {
		if (!obj[key]) {
			delete obj[key]
		}
